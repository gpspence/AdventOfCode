...
def get_new_state(
    ...
    Args:(
        ...
        list_do: re match iterable of all do() occurences
        list_dont: re match iterable of all dont() occurences
    Returns:
    ...
    if last_do == last_dont:  # If both are -1
        return state  # Keep previous state
    elif last_do > last_dont:  # Make state true
        return True
    elif last_do < last_dont:  # Make state false
        return False
    else:
...
def main(
    ...
    Args:
        matches: iterable of matches returned by re.finditer()
    Returns:
